...
public class TST<V> {
    ...
    public void countUniqueSubstrings(String input, int L) {
        for (int i = 0; i <= input.length() - L; i++) {
            String subtsr = input.substring(i, i + L);
            put(subtsr, (V) new Object());
        }
        System.out.println("Уникальные подстроки: ");
        for (String key : keys()) {
            System.out.print(key + " ");
        }
        System.out.println();
        System.out.println("Количество уникальных подстрок длиной " + L + ": " + ((Queue<String>) keys()).size());
    }
}
